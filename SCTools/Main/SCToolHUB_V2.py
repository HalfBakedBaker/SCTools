...
def openquickchat():
    ...
    # app = App_SCQuickChat()
    # app.mainloop()
    subprocess.Popen(["python", "SCQuickChat_V2.py"]) ## Better way of doing it, Seems to be more stable than trying to import the entire thing when the function is called. instead it will run the script (Must include scripts as this is a dependant method)
...
def openSCTool():
    ...
    # app = App_SCTool()
    # app.mainloop()
    subprocess.Popen(["python", "SCTool.py"]) ## Better way of doing it, Seems to be more stable than trying to import the entire thing when the function is called. instead it will run the script (Must include scripts as this is a dependant method)
...
###############################################  Redacted to improve performance. Will Show all menus at start and then toggle visibility instead of making new instances 
        ...
        self.button0 = tk.Button(web_buttons, text="ChatTool", command=lambda: openquickchat(), bg="#007E9E", fg="#FFFFFF",activebackground="#303030", activeforeground="#FFFFFF", relief="flat", bd=0, font=("Arial", 10, "bold"))
      
        self.button0.pack(side="left", padx=4, pady=1)
        # Create and pack the open SCTool button
        self.button1 = tk.Button(web_buttons, text="SCTool", command=lambda: openSCTool(), bg="#007E9E", fg="#FFFFFF", activebackground="#303030", activeforeground="#FFFFFF", relief="flat", bd=0, font=("Arial", 10, "bold"))
